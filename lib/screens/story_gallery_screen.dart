...
int _completionPercent(StorySummary story) {
  ...
  final duration = story.storyDurationMinutes <= 0 ? 1 : story.storyDurationMinutes;
      ...
      body: storiesAsync.when(
        data: (stories) {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const Text('No stories found in Firestore.', style: TextStyle(color: Colors.white54)),
          ...
          final liveStories = stories
              .where((s) => s.storyMode == StoryLifecycleMode.live || (s.storyMode == StoryLifecycleMode.scheduled && !s.isUpcoming))
              .toList();
          final upcomingStories = stories.where((s) => s.storyMode == StoryLifecycleMode.scheduled && s.isUpcoming).toList();
          final subscriptionStories = stories.where((s) => s.storyMode == StoryLifecycleMode.subscription).toList();

          return ListView(
            padding: const EdgeInsets.all(16),
            children: [
              _sectionHeader('Live'),
              ...liveStories.map((story) => _StoryCard(
                    story: story,
                    ...
                    modeChip: 'LIVE',
                    onTap: () async {
                      await ref.read(activeStoryIdProvider.notifier).setStoryId(story.id);
                      if (!context.mounted) return;
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Switched to: ${story.title}'), duration: const Duration(seconds: 2)),
                      );
                    },
                  )),
              const SizedBox(height: 12),
              _sectionHeader('Upcoming'),
              ...upcomingStories.map((story) => _StoryCard(
                    story: story,
                    isActive: story.id == activeStoryId,
                    accent: _storyColor(story.themeColorHex),
                    subtitle: story.storyStartAt == null ? 'Scheduled' : _countdownLabel(story.storyStartAt!),
                    modeChip: 'UPCOMING',
                    onTap: () async {
                      await ref.read(activeStoryIdProvider.notifier).setStoryId(story.id);
                      if (!context.mounted) return;
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Selected upcoming story: ${story.title}'), duration: const Duration(seconds: 2)),
                      );
                    },
                  )),
              const SizedBox(height: 12),
              _sectionHeader('Subscription'),
              ...subscriptionStories.map((story) => _StoryCard(
                    story: story,
                    isActive: story.id == activeStoryId,
                    accent: _storyColor(story.themeColorHex),
                    ...
                    onTap: () async {
                      ...
                      if (!context.mounted) return;
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Subscribed: ${story.title}'), duration: const Duration(seconds: 2)),
                      );
                    },
                  )),
            ],
      ...
      child: InkWell(
        ...
        child: Container(
          decoration: BoxDecoration(
            color: isActive ? accent.withOpacity(0.12) : Colors.white.withOpacity(0.05),
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
                ...
                child: Row(
                  children: [
                    Container(
                      width: 48,
                      height: 48,
                      decoration: BoxDecoration(color: accent.withOpacity(isActive ? 0.9 : 0.2), shape: BoxShape.circle),
                      child: Icon(isActive ? Icons.auto_awesome : Icons.menu_book, color: isActive ? Colors.black : accent),
                    ),
                          ...
                          Row(
                            children: [
                              Expanded(
                                child: Text(
                                  story.title,
                                  style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isActive ? accent : Colors.white),
                                ),
                              ),
                              Container(
                                ...
                                child: Text(
                                  modeChip,
                                  style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 0.8, color: accent),
                                ),
                              ),
                            ],
                          ),
                    ...
                    Icon(isActive ? Icons.check_circle : Icons.chevron_right, color: isActive ? accent : Colors.white24),
                  ],
                ),
              ),
            ],
